...
거르는 조건은 넷입니다:
    ...
    진행 상황이 게재완료가 아니다   이미 발행된 글은 새로 만들지 않습니다
...
def eligible(
    ...
    status = napi.read_select(props.get(name("status")))
    if status == cfg.notion.status_done:
        # 이미 게재된 글입니다. 지금 와서 썸네일을 붙여도 쓰일 곳이 없습니다.
        return False, "이미 게재완료"
